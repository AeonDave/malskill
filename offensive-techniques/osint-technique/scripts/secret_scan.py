...
def redact(s: str) -> str:
    """Redact secret to length + last 4 chars; never emit full material."""
    if len(s) <= 8:
        return f"<len={len(s)}>****"
    return f"<len={len(s)}>****{s[-4:]}"


def scan_text(text: str, source: str = "<stdin>"):
    """Scan a text blob; yield one dict per match. Matches are redacted by default."""
