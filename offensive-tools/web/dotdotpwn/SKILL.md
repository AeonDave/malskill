---
name: dotdotpwn
description: "DotDotPwn: directory traversal fuzzer for HTTP, FTP, and TFTP with built-in encoding variants (null byte, URL, double-URL, unicode). Use when testing path traversal vulnerabilities or generating traversal wordlists for ffuf. Multi-protocol support makes it useful for FTP/TFTP traversal where ffuf is less convenient."
license: MIT
...
dotdotpwn.pl -m ftp -h target.com -U ftpuser -P ftppass -O linux -b -s
