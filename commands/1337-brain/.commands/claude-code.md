---
description: Obsidian second brain for project knowledge - ingest sources, link atomic notes, answer grounded questions, maintain a vault via the Obsidian MCP.
argument-hint: "[init | ingest <path> | ask <q> | link <topic> | moc <topic> | dedupe | audit | project <name> | profile | log]"
---
...
Run `$ARGUMENTS` as a workflow. Classify into one of:
`init` · `ingest <path>` · `ask <question>` · `link <topic>` · `moc <topic>` · `dedupe` · `audit` · `project <name>` · `profile` · `log`. No args -> ask which.
...
- **project `<name>`**: create `wiki/projects/<name>/{Inputs,Process,Outputs,Feedback}/` + scoped `CLAUDE.md`; link to wiki + project index.
