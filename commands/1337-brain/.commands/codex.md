...
Trigger on `$1337-brain <command>` (Codex syntax) or a request to capture, organize, distill, or retrieve durable project knowledge in Obsidian. Classify into one workflow:
...
- **project `<name>`**: create `wiki/projects/<name>/{Inputs,Process,Outputs,Feedback}/` + scoped `AGENTS.md`; link to wiki + project index.
