...
- **Plugin (`opencode-background-agents`):** adds `delegate(prompt, agent)` → returns an id immediately, `delegation_read(id)`, `delegation_list()`. Results persist to disk and **survive context compaction**. Constraints: **read-only subagents only** (write-capable work must use native `task`); 15-minute timeout; background sessions sit outside the undo/branch tree. The plugin's `delegate` also accepts a **per-call `model` and `timeout`** — the one practical way to vary model per dispatch. Full setup + limits: [plugins-and-tools.md](plugins-and-tools.md).
