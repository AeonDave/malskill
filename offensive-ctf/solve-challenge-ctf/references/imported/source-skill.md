...
If CTFd is detected, **ask the user for their API token**. The token is not provided by default — the user must create one in the CTFd web UI first. Once provided, set the environment variables and proceed via API:
...
3. **Fetch links** - If the the prompt mentions URLs, fetch them FIRST for context
...
- `flag{...}`, `FLAG{...}`, `CTF{...}`, `TEAM{...}`
- Custom prefixes: check the challenge description or CTF rules for the format
...
- Prefer the token tied to the intended artifact/workflow.
