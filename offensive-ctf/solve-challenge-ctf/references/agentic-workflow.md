...
Use this reference when the concise `SKILL.md` routing is not enough and the agent needs a step-by-step solve loop for multi-category challenge triage and routing.
...
3. **Technique handoff** — load the first matching methodology skill: `recon-technique`, `forensic-technique`, `reversing-technique`, `crypto-technique`, `web-exploit-technique`.
4. **Tool selection** — choose from: `ctf-solving/*-ctf`, `coding/python-patterns`, `knowledge/systematic-debugging`, `knowledge/evidence-before-claims`.
