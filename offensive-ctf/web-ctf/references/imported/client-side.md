# Preserved source: client-side.md
...
- [JPEG+HTML Polyglot XSS]
- [JSFuck Decoding](#jsfuck-decoding)
- [AngularJS 1.x Sandbox Escape via charAt/trim Override]
...
- [jQuery `$(location.hash)` CSS Selector Timing Leak]
...
```javascript
document.querySelectorAll('[style*="display: none"], [hidden]').forEach(el => console.log(el.id, el.textContent));
...
**Pattern:** Article content is fully present in the HTML but hidden behind a CSS/JS overlay (`position: fixed; z-index: 99999; backdrop-filter: blur(...)` with a "Subscribe" CTA).
...
- `document.querySelector.remove()` in console
...
**Pattern:** File upload accepts JPEG, serves uploaded files with permissive MIME type. Admin bot visits reported files.
...
**Pattern:** Page source contains JSFuck (`[]()!+` only). Decode by removing trailing `()()` and calling `.toString()` in Node.js:
```javascript
...
**Key insight:** The sandbox relies on `charAt` to validate identifiers. Replacing it with `trim` (which returns the full string) bypasses the character-by-character check, allowing arbitrary expression evaluation. AngularJS 1.6+ removed the sandbox entirely (acknowledging it was never a security boundary), but many lab tasks and legacy apps still use older versions.

-

## Admin Bot javascript: URL Scheme Bypass

**Pattern:** Admin bot navigates to user-supplied URL, validates with `new URL()` which only checks syntax — not protocol scheme. `javascript:` URLs pass validation and execute arbitrary JS in the bot's authenticated context.
...
for (let pos = 1;; pos++) {
    for (let c of charset) {
        ...
        let sql = sqlTemp.replace('__LEFT__', readQuery).replace('__RIGHT__', `'${c}'`);
