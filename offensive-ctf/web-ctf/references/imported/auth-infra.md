# Preserved source: auth-infra.md
...
- [CI/CD Variable Credential Theft]
- [Identity Provider API Takeover]
- [SAML SSO Flow Automation]
- [Apache Guacamole Connection Parameter Extraction]
- [Login Page Poisoning for Credential Harvesting]
- [TeamCity REST API RCE]
...
*See also [ctf-crypto/modern-ciphers-2.md — Hash Length Extension Attack] for the canonical crypto writeup of the same primitive.*
...
**Key insight:** Any Merkle-Damgård hash used as `H(secret || data)` without HMAC construction leaks internal state at the message boundary, enabling arbitrary message extension. Use `hashpumpy` or `hash_extender`. If the secret length is unknown, brute-force it — the valid extension will produce a server-accepted MAC.
