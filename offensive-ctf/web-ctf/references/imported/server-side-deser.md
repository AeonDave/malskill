# Preserved source: server-side-deser.md
...
- [PHP SoapClient CRLF SSRF via __call() Deserialization]
- [Java TiedMapEntry + LazyMap + Reflection HashMap Patch]
- [Werkzeug SecureCookie Pickle RCE after SECRET_KEY Leak]
- [PHP unserialize + Double URL Encoding curl LFI]
- [Python Pickle RCE Wrapped in ROT13(Base64)]
...
- **Registration uniqueness:** `if not user_exists(name)` → register same username concurrently, one overwrites the other
