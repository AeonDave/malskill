# Preserved source: server-side-advanced.md
...
- [/dev/fd Symlink to Bypass /proc Filter]
- [Unicode Homoglyph Path Traversal U+2E2E]
- [Ruby Regexp.escape Multibyte Character Bypass]
...
**Pattern:** Go validates `len([]rune(input)) > 32` but copies `len([]byte(input))` bytes.
...
**Pattern:** Server extracts uploaded ZIP without checking symlinks.
...
GET /laravel../app/Http/Controllers/Augeneric caseontroller.php HTTP/1.1
...
```python
# In a lab context: HTTP request with GBK lead byte in parameter
import requests
...
**Pattern:** Flask app with Werkzeug debugger enabled + weak session secret.
...
**Pattern:** Upload endpoint filters keywords ("file", "flag", "etc") in uploaded XML, but external DTD fetched via HTTP is NOT filtered.
