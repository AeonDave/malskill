# Preserved source: server-side-exec-2.md
...
- [Wget GET Parameter Filename Trick for PHP Shell Upload]
- [Tar Filename Command Injection]
- [PNG/PHP Polyglot Upload + Double Extension + disable_functions Bypass]
- [PHP BMP Pixel Webshell with Filename Truncation]
- [Editor Backup File Source Disclosure]
- [date -f Arbitrary File Read]
- [Apache mod_rewrite PATH_INFO Bypass]
- [PHP ReDoS to Skip Code Execution]
- [Custom Serializer Integer Overflow 256 to 0 Length]
- [Pickle Chaining via STOP Opcode Stripping] *(stub — see [server-side-deser.md](server-side-deser.md))*
...
- [Unanchored Regex Command Injection]
- [Jinja2 SSTI via globals.__self__.exec() String Concat Bypass]
- [web.py reparam() eval + __subclasses__ with Blanked Builtins]
- [Redis Lua Injection via redis.call()]
- [PHP create_function String Interpolation RCE]
- [php://input + NULL Byte + ~Bitwise base64 Filter Bypass]
- [EXIF ImageDescription Shell Injection via exiftool]
- [.phar Extension Bypass for PHP Upload Blacklists]
...
**Pattern:** Multi-stage chain: SQLi file write + PHP7 OPcache poisoning + `LD_PRELOAD` bypass of `disable_functions`.
...
**Pattern:** Server uses `wget` to download user-provided URLs and `parse_url()` to validate the path. Wget saves files with GET parameters in the filename, creating a `.php` extension bypass.
...
**Pattern:** Server extracts tar archives and displays filenames via a `.cgi` script. Filenames containing shell metacharacters are passed to shell without sanitization.
...
**Pattern:** Upload filter rejects `.php` extension but accepts image uploads. nginx/PHP-FPM executes files ending in `.php` regardless of preceding extensions. `disable_functions` blocks all command execution functions, but filesystem functions remain available.
...
$files = scandir('/');
foreach ($files as $f) {
    if (strpos($f, 'flag') !== false || strpos !== false) {
        echo "FOUND: $f\n";
...
**Key insight:** Any PHP function that builds code
...
**Pattern:** vsftpd 2.3.4 shipped with a compromised source release: any username ending in `:)` triggers a bind shell on TCP port 6200. Detect the vulnerable version via FTP banner or service fingerprint; trigger the backdoor; connect to port 6200 for root shell.
