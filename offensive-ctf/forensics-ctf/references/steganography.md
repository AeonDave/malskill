# Preserved source: steganography.md
...
**Pattern:** Message encoded as black/white pixels in 1-pixel border around image.
...
**Pattern:** Flag hidden across multiple layers in a PDF.
...
**Pattern:** SVG favicon contains animation keyframes with alternating fill colors.
...
**Pattern:** Invalid PNG has chunks out of order.
...
**Pattern:** Archive appended after PNG IEND, but magic bytes overwritten with PNG signature.
...
**Pattern:** Each PNG layer XOR-encrypted with incrementing keys ("layer2", "layer3", etc.).
...
**Pattern:** A file contains Kitty terminal graphics protocol escape sequences (`ESC_G`) that embed zlib-compressed RGB image data in base64-encoded chunks.
...
**Detection:** Binary file containing `\x1b_G` sequences. `strings` output shows base64-like data interspersed with escape codes. the prompt mentions "kitty", "terminal graphics", or "meow".

-

## ANSI Escape Sequence Steganography in Terminal Art

**Pattern:** Flag text is interleaved between ANSI color escape codes and Unicode braille characters in terminal art. When rendered in a terminal, the art displays normally while the flag characters are invisible (zero-width or same-color-as-background). However, the flag is extractable by stripping all escape sequences and non-ASCII characters.
...
**Detection:** File with many `\x1b[` sequences (ANSI codes), Unicode braille characters (U+2800-U+28FF), and unexpectedly large file size for the visible content. the prompt mentions "terminal", "art", "ANSI", or shows ASCII/Unicode art.

-

### Autostereogram / Magic Eye Solving

**Pattern:** Challenge image is an autostereogram (Magic Eye). The hidden 3D content (flag text) is revealed by viewing with crossed/gent eyes or programmatically via layer difference.
...
**When to recognize:** Image has a repeating texture/pattern, the prompt mentions "eyes", "seeing", "3D", "magic", or "stereogram".

-

### Two-Layer Byte+Line Interleaving

**Pattern:** Two PNG files are interleaved at the byte level into a single file. After byte-level deinterlacing, the resulting images have their scanlines interleaved, requiring a second round of line-level deinterlacing.
...
**Pattern:** An MP4 video container holds multiple video streams. The default (stream 0:0) plays normally, but a second stream (0:1) contains the flag. Most video players only show the first/default stream. The secondary stream uses AV1 codec which has poor support in many tools, adding friction.
...
**When to recognize:** Artifact set provides a video file where the visible content seems irrelevant or is a red herring. `ffprobe` shows multiple `Stream` entries. Check metadata fields like `handler_name` for hints.
...
**Pattern:** PNG contains standard `IDAT` chunk (coarse first scan) plus custom `scRT` chunks. Each `scRT` chunk is XOR-encrypted with a multi-byte key. Decrypting reveals another `IDAT` chunk plus another `scRT`, forming nested layers.
...
5. When initial decode fails, use known plaintext to fix first data bytes
6. High ECC level corrects remaining pixel errors
