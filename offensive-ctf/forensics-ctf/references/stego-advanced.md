# Preserved source: stego-advanced.md
...
**Pattern:** Image encodes data in frequency domain via 2D FFT.
...
**Identification:** the prompt mentions "transform", poem about "frequency", or image looks blank/noisy. Try FFT visualization first.

-

## SSTV Red Herring + LSB Audio Stego

**Pattern:** WAV contains SSTV signal (Scottie 1) that decodes to "SEEMS LIKE A DEADEND". Real flag in 2-bit LSB of audio samples.
...
**Pattern:** SSTV decoding produces dot pattern image. Not QR — it's DotCode format.
...
**Pattern:** Audio file contains phone dialing tones encoding data.
...
**Pattern:** Audio with dual-tone sequences at non-standard frequencies, aligned at regular intervals (e.g., every 1 second). Hints about "harmonic oscillators" or physics point to custom frequency design.
...
**Pattern:** MKV/video file with two nearly-identical audio tracks. Hidden data is embedded as a tiny difference between the tracks, invisible when listening to either individually.
...
- Flag may only be visible in a narrow time window — use high-resolution spectrogram

-

## Cross-Channel Multi-Bit LSB Steganography

**Pattern:** Standard LSB tools (zsteg, stegsolve) fail because different bit positions are used per RGB channel: Red channel bit 0, Green channel bit 1, Blue channel bit 2.
...
**Pattern:** Identify dominant frequencies via FFT (Fast Fourier Transform), map to musical notes (A-G), then read the letter names as a word.
...
**Pattern:** Audio file metadata (exiftool comment field) contains underscore-separated numbers representing octal-encoded ASCII values (digits 0-7 only).
...
**Pattern:** Deeply nested tar archives where data is encoded in whitespace characters (spaces, tabs, newlines) within file names or content.
...
**Detection:** WAV file that seems normal but `deepsound2john.py` produces a hash. Challenge has two-part structure where first part is easy (spectrogram) and second part requires a tool. the prompt mentions "layers", "hidden", or "deep".
...
**Pattern:** Distributed file is a valid `.docx` (ZIP archive). Extract it normally and you see only a decoy document. Reverse the entire file byte-for-byte and the result is *also* a valid ZIP archive — containing a second `word/media/*.png` whose contents are the flag.
