# Preserved source: signals-and-hardware.md
...
**Identification:** Binary data with 10-bit aligned structure. the prompt mentions HDMI, DVI, or TMDS.

-

## DisplayPort 8b/10b + LFSR Decoding

**Structure:** 10-bit 8b/10b symbols decoded to 8-bit data, then LFSR-descrambled. Organized in 64-column Transport Units.
...
**Pattern:** Analog image encoded as audio. Sync pulses (sharp negative spikes) delimit scan lines. Amplitude between pulses = pixel brightness.
...
**Pattern:** Power consumption traces recorded during cryptographic operations. Correct key guesses cause measurably different power consumption at specific sample points.
...
**Identification:** the prompt mentions "power", "side-channel", "leakage", "traces", or "measurements". Data is a multi-dimensional numeric array with axes for positions/guesses/traces/samples.
...
**Pattern:** Saleae Logic 2 `.sal` file (ZIP archive) containing digital channel captures. Data encoded as UART serial.
...
initial_state: u32 (0 or 1)... header fields...
...
**Pattern:** Recover typed text from audio recordings of keystrokes. Reference audio provides labeled samples (known keys), flag audio contains unknown keystrokes to classify.
...
**Detection:** Two audio files provided (reference + target), or the prompt mentions "typing", "keyboard", "acoustic".
...
**Calibration workflow:** The Artifact set provides `calibrate_img.cdda` with a known output (`calibrate_img.png` showing "Calibrate: 0123456789abc..."). Use this pair to verify geometry parameters (tr0, dtr, r0, scale) before decoding the flag file.

**Detection:** the prompt mentions "album", "CD rip", "CDDA", or provides large (~800MB) files with only 2 unique byte values. The `file` command reports "ISO-8859 text with CR line terminators" because `0x0d` (CR) is one of the two values.
...
**Detection:** Video of a keyboard where an LED blinks irregularly. the prompt mentions "security camera", "keyboard", "blinking", or "Morse".
...
**Detection:** Binary file size divisible by 24. the prompt mentions keylogger, keyboard, or input device.
...
**Detection:** WAV file with a clean square wave pattern visible in Audacity. Two distinct amplitude levels with regular timing. the prompt mentions "serial", "UART", "baud", or "RS-232".
...
**Detection:** USB PCAP with bulk transfer packets containing 3-byte or 4-byte payloads. USB device descriptor shows MIDI class (Audio class, subclass MIDI Streaming). the prompt mentions "MIDI", "Launchpad", "music controller", or "grid".
