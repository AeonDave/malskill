# Preserved source: linux-forensics.md
...
**Pattern:** Full attack timeline from logs + PCAP + malware.
...
**Pattern:** Flag hidden in browser history URLs.

```bash
# Quick method
strings places.sqlite | grep -i "flag\|"
...
**Pattern:** USB isochronous transfers contain audio data.
...
**Pattern:** TLS 1.2 with `TLS_RSA_WITH_AES_256_CBC_SHA` (no PFS).
...
**Pattern:** KeePass database (`.kdbx`) found on compromised system contains SSH keys or credentials for lateral movement.
...
cd keepass2john && make./keepass2john system.kdbx > hash.txt
...
**Pattern:** Git repository with clean history where data was overwritten and history rewritten via `git rebase -squash`. The original commits survive as orphaned objects.
...
**Detection:** Git repo with suspiciously clean history (single commit, or squash-merge commits). the prompt mentions "rewrite", "rebase", "squash", or "clean history".
...
**Pattern:** Git repository with corrupted blob objects. Since git identifies objects by SHA-1 hash, a single-byte corruption changes the hash, making the object unreadable. Repair by brute-forcing each byte position until `git hash-object` produces the expected hash.
...
# - Chainalysis Reactor: commercial but referenced in lab tasks
...
**Detection:** Artifact set provides access to a running system where a Python process is active but the `.py` source file has been deleted. `ls -l /proc/<PID>/exe` shows the Python interpreter; `/proc/<PID>/fd/` may still reference the deleted file. Check `ptrace` permissions (`/proc/sys/kernel/yama/ptrace_scope`).
