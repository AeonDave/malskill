# Preserved source: disk-recovery.md
...
Companion tools: `rsakeyfind` (RSA keys), `aesfix`.
...
# Decompress using provided model./fzip -model fashion.model -decompress compressed_dir/ -output decompressed_dir/
```
...
**Key insight:** FemtoZip is rare in lab tasks. Identify it by the `.model` file and the presence of many small compressed files that share common structure (JSON, XML templates).
...
**Pattern:** Deleted files on a BTRFS filesystem may persist in snapshots or alternate subvolumes. The default mount shows only the active subvolume, but backup snapshots contain historical file states.
...
**Detection:** `file disk.img` shows "BTRFS Filesystem". the prompt mentions "snapshots", "time travel", "turn back", or "recovery".

-

## FAT16 Free Space Data Recovery

**Pattern:** Data is hidden in the free (unallocated) clusters of a FAT16 filesystem. The mounted filesystem shows no suspicious files, but free clusters contain recoverable data.
...
**When to recognize:** Artifact set provides a filesystem image. Mounting shows nothing useful, but `file` identifies it as FAT16/FAT32. Volume label or challenge description hints at "free space", "deleted", or "hidden in plain sight".

-

## FAT16 Deleted File Recovery via Sleuth Kit

**Pattern:** A file has been deleted from a FAT16 filesystem image. The file's data and cluster chain remain intact, but the directory entry's first byte is replaced with `0xE5` (the FAT deletion marker). Sleuth Kit's `fls` and `icat` recover the file by inode.
...
**When to recognize:** Artifact set provides a FAT filesystem image with a deleted file. The prompt wording or artifact context hints at deletion (`rm`, `deleted`, `removed`). Mount shows the file is missing, but `fls` reveals the deleted directory entry.
...
**Pattern:** A file has been deleted from an ext2 filesystem, leaving an orphaned inode. The file doesn't appear in any directory listing, but `fsck` detects the unattached inode and can reconnect it to `/lost+found`.
...
**When to recognize:** Artifact set provides an ext2/ext3/ext4 filesystem image. Normal mounting shows nothing. Challenge hints at "deleted", "orphan", "lost", or "recovery". Always run `fsck` on forensics filesystem images.
