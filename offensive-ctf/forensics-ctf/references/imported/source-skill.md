...
Quick reference for forensics challenges. Each technique has a one-liner here; see supporting files for full details.
...
- [network-advanced.md](network-advanced.md) - Advanced network forensics
- [peripheral-capture.md](peripheral-capture.md) - USB/HID/Bluetooth peripheral traffic reconstruction
...
- [steganography.md](steganography.md) - General steganography
...
- [stego-advanced.md](stego-advanced.md) - Advanced steganography part 1: audio and signal techniques
- [stego-advanced-2.md](stego-advanced-2.md) - Advanced steganography part 2: video, image transform, and format-specific techniques
- [linux-forensics.md](linux-forensics.md) - Linux/app forensics, corrupted git blob repair via byte brute-force, VBA macro Excel cell data to ELF binary extraction, Python in-memory source recovery via pyrasite)
...
- If you need to trace infrastructure, attribute actors, or investigate public records
...
**Advanced PDF stego:** Six techniques - invisible text separators, URI annotations with escaped braces, Wiener deconvolution on blurred images, vector rectangle QR codes, compressed object streams (`mutool clean -d`), document metadata fields.
...
**Pattern:** Malware uses WMI event subscriptions for persistence (MITRE T1546.003).
...
- **WPA/WEP WiFi decryption:** `aircrack-ng -w wordlist capture.pcap` cracks WPA handshake; WEP cracked with enough IVs. See [network.md].
- **PCAP repair:** `pcapfix -d corrupted.pcap` repairs broken PCAP headers/checksums for Wireshark loading. See [network.md].
...
- **GIMP raw memory dump visual inspection:** When Volatility fails, open `.dmp` in GIMP as raw RGB data at monitor width; scroll to find framebuffer screenshots of user's desktop. See [disk-and-memory.md].
