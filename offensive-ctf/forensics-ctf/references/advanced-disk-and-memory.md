# Preserved source: disk-advanced.md
...
**Pattern:** USB image with deleted partition table.
...
**Flag hiding:** Path components as flag chars
...
**Pattern:** "LLMs only" + "undercut" → not AI GPT, but GUID Partition Table.
...
**Pattern:** Go binary crash dump. Flag as plaintext string constant in.data section survives in minidump memory.

```bash
strings -a minidump.dmp | grep -i "flag\|ctf\|"
```
...
**Pattern:** Split sparse VMDK requires grain directory + grain table traversal.
...
**Pattern:** Linux memory dump with flag in environment variables or process data.
...
**Pattern:** Extract fake executable from Windows memory dump. Cipher: subtract 0x32, then XOR with cycling key (large multi-line string, e.g., ASCII art).
...
**Pattern:** Corel Linux disk image containing WordPerfect macro file (fc.wcm) with XOR-encrypted byte arrays.
...
**Pattern:** Two relics in volatile memory (minidump) must be XORed; ISO 9660 directory entries in memory fragments point to hidden data.
...
**Pattern:** APFS volume maintains historical snapshots; recovering earlier state of a key file reveals authentic value before poisoning.
...
**Detection:** Artifact set provides multiple disk images of identical size, mentions "array", "redundancy", or "parity". `file` command may identify them as filesystem images or raw data.
...
**Detection:** DMG or HFS+ disk image where standard carving finds nothing. `file` identifies as "Apple HFS+" or "Apple Partition Map". the prompt mentions "Mac", "Apple", or "hidden data".
...
**Detection:** SQLite database with tables containing `type`/`operation`, `position`, `text` columns. the prompt mentions "notes", "editor", "collaboration", or "history". Schema inspection via `.schema` or `sqlite3 db.sqlite ".tables"` reveals diff-style tables.
