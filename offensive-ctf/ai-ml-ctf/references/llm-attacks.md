...
**Key insight:** LLMs cannot reliably distinguish between instructions and data. Any text the model processes can potentially override its behavior. System prompts are not a security boundary. In challenges, try every injection surface: user input, file uploads, URLs the bot fetches, tool outputs, even filenames and metadata.
...
**Key insight:** Safety alignment is probabilistic, not absolute. Different techniques work on different models and configurations. Always try multiple approaches. Multi-turn attacks are often more effective than single-turn because they gradually shift the model's context. In challenges, the "safety filter" is often a simple keyword check or a system prompt instruction, both of which are much easier to bypass than production safety systems.
