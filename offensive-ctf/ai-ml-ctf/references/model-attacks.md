# Preserved source: model-attacks.md
...
**Pattern:** A GPT-2 model has been fine-tuned to suppress a specific behavior (e.g., generating a flag). The Artifact set provides both the original base model and the fine-tuned (suppressed) model. By computing the weight delta and negating it, you reverse the suppression into amplification.
...
**Pattern:** A challenge exposes a model via an API endpoint. By sending carefully crafted inputs and observing outputs, you can reconstruct the model's parameters or decision boundary. This is especially effective against simple models (linear, decision tree, small neural networks).
...
**Key insight:** Linear models can be extracted exactly with `dim + 1` queries. For neural networks, use model distillation: train a student network on input-output pairs from the API. Decision trees can be extracted by probing decision boundaries with binary search.

**Variations:**
- **Logit extraction:** If API returns only class labels, use inputs near decision boundaries with binary search.
...
**Pattern:** Determine whether a specific data sample was part of the model's training set. Training data members typically produce higher predictions and lower loss values than non-members, because the model has memorized them to some degree.
...
**Key insight:** Models overfit to training data, producing measurably different behavior on seen vs. unseen samples. The gap between training and test is the core signal. More sophisticated attacks train a binary classifier on (metrics, member/non-member) pairs from shadow models trained on similar data distributions.

**Variations:**
- **Label-only attack:** When only the predicted class is returned, use perturbation sensitivity: members are more robust to small perturbations.
