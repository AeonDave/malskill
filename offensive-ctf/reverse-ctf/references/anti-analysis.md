# Preserved source: anti-analysis.md
...
Comprehensive reference for anti-debugging, anti-VM, anti-DBI, and integrity-check techniques encountered in challenges, with practical bypasses.
...
```asm; Condition that always evaluates the same way but looks data-dependent
mov eax, [some_memory]
imul eax, eax; x^2
and eax, 1; x^2 mod 2 is always 0 for any x
jnz fake_branch; Never taken, but disassembler doesn't know; real code here
```
...
```asm; Jumps into the middle of a multi-byte instruction
...
See [anti-analysis-ctf.md] for CTF writeup techniques: SIGILL handler for mode switching, SIGFPE strace side-channel, instruction trace inversion, call-less function chaining, and parent-patched child binary dump via `process_vm_writev`.
