# Preserved source: tools-dynamic.md
...
- [x64dbg (Windows Debugger)](#x64dbg-windows-debugger)
  ...
  - [Scripting](#scripting)
  - [Common CTF Workflow]
- [GDB Register Side-Channel on putchar()]
- [radare2 Visual Panels for Custom VM Tracing]
- [libSegFault.so Register Dump at Crash]
- [r2pipe Binary Walking + DP Constraint Solver]
- [GDB Commands at strcmp to Recover Dynamic XOR Key]
...
**Pattern:** Binary uses the [obfy](https://github.com/fritzone/obfy) C++ template obfuscator to bury a simple `enc(input)` XOR loop under thousands of opaque predicates. The terminal check is still `strcmp(expected_ciphertext, enc(input))` — so instead of unwinding obfy, break at the `strcmp` call and dump both operands:
