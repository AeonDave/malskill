# Preserved source: languages-platforms.md
...
- [Electron App + Native Binary Reversing]
- [Node.js npm Package Runtime Introspection]
...
- [Glulx Interactive Fiction Bytecode Matrix Validation]
- [Android Smali Injection to Defeat LocalBroadcastManager]
...
**Pattern:** Roblox game with flag hidden in older version; latest version contains decoy.
...
**Pattern:** Encrypted Godot.pck packages.
...
**Pattern:** Rust binary reads JSON from stdin, deserializes via serde_json, prints success/failure emoji.
...
3. Look for string literals in deserializer code
...
```java
// Create new project with same package: com.example.puing.a2018public source
package com.example.puing.a2018public source;
...
**Pattern:** Android app validates actions (QR codes, purchases) via Firebase Cloud Functions. The expected payload format includes the Firebase UID, a value, and a timestamp. Use Frida to hook the app post-login, construct a valid payload, and call the Cloud Function directly.

```javascript
// Frida hook to bypass QR validation
Java.perform(function() {
    var FirebaseFunctions = Java.use;
    var FirebaseAuth = Java.use;
...
**Pattern:** Verilog HDL source for a vending machine with hidden product unlocked by specific coin insertion and selection sequence.
...
**Pattern:** A single file valid in both Ruby and Perl. Each language imposes different validation constraints on a 50-character key. Satisfy both simultaneously to decrypt the flag.
...
**Detection:** File that runs under multiple interpreters (`ruby file && perl file`). the prompt mentions "polyglot" or provides a file ending in `.rb` that also looks like Perl.

-

## Electron App + Native Binary Reversing

**Pattern:** Electron desktop app bundles a native ELF/DLL binary for sensitive operations (vault, crypto, auth). The Electron layer is a wrapper; the real flag logic is in the native binary.
...
**Pattern:** Obfuscated npm package with RC4 encoding, control flow flattening, and flag split across multiple fragments. Static analysis is impractical — use runtime introspection instead.
...
const hidden = Object.getOwnPropertyNames(Engine).filter(p => p.startsWith('__') || p.startsWith('_'));
...
**Pattern:** APK registers a `BroadcastReceiver` through `LocalBroadcastManager.getInstance(this).registerReceiver(...)` that only decodes the flag when triggered by the app itself. External `adb shell am broadcast` is silently dropped because local broadcasts never leave the process:

```bash
$ adb shell cmd package query-receivers -brief -a com.public source.ctf.START
# No receivers found
```
