# Preserved source: languages-compiled.md
...
- [Go Binary Reversing](#go-binary-reversing)
  ...
  - [Go Binary UUID Patching for C2 Client Enumeration]
- [Rust Binary Reversing](#rust-binary-reversing)
  ...
  - [Rust Lifetime Escape via Compiler Bug #25860]
  - [Rust #[no_mangle] libc Override for seccomp Bypass]
  - [Rust xmmword Constant Extraction via IDAPython]
- [Nuitka-Compiled Python — Module Stub Injection]
...
Go binaries are increasingly common in challenges due to Go's popularity for CLI tools, network services, and malware.
...
# https://github.com/mandiant/GoReSym./GoReSym -d binary > symbols.json
...
**Common stdlib patterns In lab-style tasks:**
```go
...
**Pattern:** A Go-compiled C2 client has a UUID embedded via `-ldflags -X`. The C2 server uses mTLS for authentication. To enumerate other clients and their files, patch the UUID to register as a new client, then use the C2 API to list all clients and download their exfiltrated files.
...
"
chmod +x client_patched./client_patched -register
```
...
Rust binaries are common in modern lab tasks, especially for crypto, systems, and security tooling challenges.
...
**Key insight:** D binaries are rare in lab tasks but identifiable by `_D` symbol prefixes and Phobos library references. The compile-time template system means D functions may be duplicated hundreds of times with different parameters — look for patterns like `enc!("N")` where N varies.
...
**Key insight:** Any runtime that still resolves module names through `sys.path` can be shimmed at import time with CWD stubs. Grep `strings` output for module names to pick good hook targets.
