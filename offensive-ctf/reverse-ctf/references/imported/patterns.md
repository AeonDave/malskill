...
- [Anti-Debugging Techniques](#anti-debugging-techniques)
  ...
  - [pwntools Binary Patching](#pwntools-binary-patching)
- [Nanomites](#nanomites)
...
**Pattern:** Encrypted bytes given; flag format known.
...
known = b"public source{say_yes_to_me"
...
loop_top:; State for iterations 2+
    mov  r13, sbox[a & 0xf]; Uses OLD 'a', not new!

loop_middle:; Main computation
    inc  a
    jne  loop_top
```
...
**Pattern:** Binary mangles input 2 bytes at a time with intermediate state, compares to static target.
...
**Pattern:** Input converted to hex, compared against hex constant.
...
**Pattern:** Binary uses UNIX signals as a binary tree navigation mechanism.
