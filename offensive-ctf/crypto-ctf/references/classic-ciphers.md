...
- [XOR Key Recovery via File Format Headers]
- [3D Vigenere Palindrome Symmetry Key Recovery]
- [Nihilist Cipher Double-Crib Key Recovery]
- [16-Byte XOR Block Cipher Structural Reversal]
- [Flag Semaphore Photo Decoding]
- [Two-Byte Nibble Reassembly with Random Padding]
...
**Identification:** Prompt wording hints ("Abashed" = Atbash), preserves spaces/punctuation, 1-to-1 substitution.
...
**Pattern:** Physical cipher wheel with inner/outer alphabets.
...
**Pattern:** Each byte XORed with previous ciphertext byte.
...
**Pattern:** Given `S XOR ROTR(S, k)` for multiple rotation offsets k, recover S.
...
**Pattern:** Verification XORs all comparison bytes into a single byte instead of checking each individually.
...
**Pattern:** Service encrypts data with deterministic keystream that resets per connection. Multiple backends with different keystreams behind a load balancer.
...
**Pattern:** Two ciphertexts encrypted with the same OTP key. Known plaintext for one message enables recovery of the other.
...
**Pattern:** Book cipher with "steps forward" encoding. Brute-force starting position with charset filtering reduces ~56k candidates to 3-4.
...
**Pattern:** Ciphertext uses alphanumeric characters grouped in blocks of 5. Single-character frequency analysis shows non-uniform distribution. N-gram analysis reveals repeated multi-character groups mapping to single plaintext characters, with different plaintext characters encoded by groups of different lengths (1-4 characters).
...
for p in permutations(ambiguous_chars):
    ...
    if sha256.encode()).hexdigest() == known_hash:
        print
        break
```
...
**When to recognize:** Artifact set provides one or two image files with visible horizontal or vertical "shearing" artifacts. If an original image is provided alongside a shifted version, compute per-row or per-column offsets and check if they decode as ASCII.

-

## XOR Key Recovery via File Format Headers

**Pattern:** A file claims to be a known format (e.g., PDF, PNG, ZIP) but `file` reports it as "data". The file has been XOR-encrypted with a repeating key. Recover the key by XOR-ing the encrypted bytes against the expected file format header, then extend the key using known structural elements at the end of the file.
...
**When to recognize:** Artifact set provides a file that should be a known format (filename extension or description says so) but `file` reports "data" or wrong type. Hex dump shows no recognizable magic bytes. XOR the first few bytes against the expected header - if the result looks like an ASCII string or repeating pattern, it is a repeating XOR key.
...
**Pattern:** Nihilist cipher encrypts via Polybius square lookup + additive numeric key. Each plaintext letter becomes a two-digit Polybius coordinate, then the key's digit stream is added to produce ciphertext digits.
...
**Pattern:** Encryption operates on 16-byte blocks split into four 4-byte lanes. Each output byte is an XOR of several input bytes from the same block; one lane can be recovered as the XOR of the other three.
...
**Pattern:** Artifact set provides photos of a person holding flags in fixed positions. Map each pose to a letter using the standard flag semaphore chart (8 compass directions × 2 arms = ~32 letters). Two special poses — `J` (shift to "letters") and `#` (shift to "numbers") — switch between modes; maintain a mode flag while decoding.
