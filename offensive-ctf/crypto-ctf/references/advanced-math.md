# Preserved source: advanced-math.md
...
- [Clock Group (x^2+y^2=1 mod p) DLP]
...
- [LWE Lattice Attack via CVP]
- [Affine Cipher over Non-Prime Modulus]
- [Introspective CRC via GF(2) Linear Algebra]
- [Baby-Step Giant-Step for Sparse/Low Hamming Weight Exponents]
- [Hensel's Lemma: Polynomial Root Lifting mod p^k]
...
**Pattern:** Server gives hints `h_i = f * p_i + n_i` where f is the flag, p_i are small primes, n_i is small noise.
...
**Pattern:** Two RSA key pairs with d1 ≈ d2 (small difference).
...
**Pattern:** p = base + 10^k · x where base is fully known, x is small.
...
**Pattern:** Diffie-Hellman on the unit circle group.
...
**Pattern:** Multi-layer challenge ending with Learning With Errors (LWE) recovery. Secret vector `s` in {-1, 0, 1}^n, public matrix A, ciphertext `b = A*s + e (mod q)`.
...
**Pattern:** Challenge gives a polynomial `P(x)` whose unique root mod `N = p^k` is the flag, where `p` is a small known prime and `k` is large (e.g. `p ~ 2^16`, `k = 100`). Brute force over `p^k` is hopeless, but Hensel's lemma lifts any simple root mod `p` to a unique root mod `p^k` via Newton iteration: given `P(r) ≡ 0 mod p^i`, the lift is `r' = r - P(r) * inverse(P'(r), p) mod p^(i+1)`. Factor out the intermediate reductions to `mod p^(i+1)` each step or the integers blow up exponentially.
