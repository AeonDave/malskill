...
- [Fibonacci Stream Cipher Position-Shifting Oracle]
- [Z3 Constraint Solving for Custom Stream Ciphers]
- [Keystream Recovery via Run-Length Encoding Collisions]
- [LFSR Filter Linear Annihilator Attack]
- [Hostname-as-XOR-Key Leaked via DNS Capture]

-

## LFSR Stream Cipher Attacks

Linear Feedback Shift Registers generate keystreams
...
- **Fibonacci:** feedback from multiple taps XOR'd into last position
...
**Pattern:** A PNG file is encrypted by XORing each N-bit block with the current state of a Galois LFSR (right-shift model). The LFSR length, seed, and tap mask are unknown. Recover all three from the known 16-byte PNG header.
...
**Pattern:** Custom stream cipher with algebraic mixing: `encrypted[i] = (message[i] + key[i%13] + encrypted[i-1]) % 128`. Known plaintext prefix anchors the first several constraints. Z3 solver encodes each step as an integer constraint and directly recovers both the unknown key and the remaining flag characters.
