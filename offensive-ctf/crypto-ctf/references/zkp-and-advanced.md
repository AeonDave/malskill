# Preserved source: zkp-and-advanced.md
...
- [Post-Quantum Signature Fault Injection: MAYO]
- [Lattice-Based Threshold Signature Attack: FROST]
- [Groth16 Broken Trusted Setup — delta == gamma]
- [Groth16 Proof Replay — Unconstrained Nullifier]
...
**Pattern:** Yao's garbled circuit with free XOR optimization. Circuit designed so normal evaluation only reaches one wire label, but the other is needed.
...
**Pattern:** Bigram substitution cipher where plaintext has known structure (NATO phonetic alphabet).
...
**Pattern:** "Trigram substitution" that decomposes into three independent monoalphabetic ciphers on positions mod 3.
...
**Pattern:** Coefficients `a_1...a_9` are deterministic functions of secret s (via RNG seeded with s). One share (x_0, y_0) is revealed.
...
**Pattern:** Endpoint has TOCTOU vulnerability: `if counter < 4` check happens before increment, allowing concurrent requests to all pass the check.
...
**Pattern:** Service evaluates AES via garbled circuits with a fixed per-connection key. Exploit garbling metadata rather than AES cryptanalysis.
...
**Key insight:** Garbled circuits that use free XOR optimization with fixed keys across sessions leak key material through the AND gate truth tables. Each S-box has a small enough input space (256 values) to brute-force when you know delta. This extends the technique from "recovering delta" to "recovering the entire AES key."

-

## Post-Quantum Signature Fault Injection: MAYO

**Pattern:** One-byte fault injection window in `mayo_sign_signature` before final `s = v + O*x` construction. Controlled bit flips across 64 signature queries recover the secret matrix O row by row.
...
**Pattern:** Preprocessing queue capacity allows collecting many signatures. Fixed challenge construction enables solving 1D noisy linear equations per coefficient.
...
**Pattern:** Groth16 verifier has `vk_delta_2 == vk_gamma_2`, which breaks soundness entirely. Proofs are trivially forgeable.
...
**Pattern:** DAO governance never tracks used `proposalNullifierHash` values, and the circuit leaves the nullifier unconstrained. A valid proof from the setup transaction can be replayed infinitely.
...
**Pattern:** DV-SNARG (Designated Verifier Succinct Non-interactive ARGument) for an adder circuit. Must produce 20 valid proofs for **wrong** answers.

**Key insight:** DV-SNARGs explicitly lose soundness when the prover has oracle access to the verifier. The verifier's secret randomness can be extracted through query patterns.
...
**Pattern:** KZG commitment scheme publishes shuffled points `{alpha^i * G1}` for i=0..n. The shuffle hides which point corresponds to which exponent. Recover the exponent ordering using bilinear pairings as an oracle, then extract the toxic waste `alpha`.
