...
- [rsa-attacks-2.md: RSA p=q Validation Bypass]
- [rsa-attacks-2.md: RSA Cube Root CRT when gcd(e, phi) > 1]
- [rsa-attacks-2.md: Factoring n from Multiple of phi(n)]
- [rsa-attacks-2.md: RSA Signature Forgery via Multiplicative Homomorphism]
- [rsa-attacks-2.md: Weak RSA Key Generation via Base Representation]
- [rsa-attacks-2.md: RSA with gcd(e, phi(n)) > 1]
- [rsa-attacks-2.md: Batch GCD for Shared Prime Factoring]
- [rsa-attacks-2.md: RSA Partial Key Recovery from dp dq qinv]
- [rsa-attacks-2.md: RSA-CRT Fault Attack / Bit-Flip Recovery]
- [rsa-attacks-2.md: RSA Homomorphic Decryption Oracle Bypass]
- [rsa-attacks-2.md: RSA with Small Prime Factors and CRT Decomposition]
...
**Pattern:** RSA signature scheme using polynomial hash `g(x,a,b) = x(x^2 + ax + b) mod P`.
...
**Pattern:** `r = flag mod f` where f is random GF(2) polynomial. Collect ~20 pairs, filter coprime, CRT combine.
...
**Pattern:** `c = A @ p + b (mod m)` with composite m. Chosen-plaintext difference attack. For composite modulus, solve via CRT in each prime factor field separately.
