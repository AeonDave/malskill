...
- [EC-LCG State Recovery from Truncated Coordinates]
...
**Pattern:** A service exposes IDs or tokens derived from high bits of elliptic-curve point coordinates generated by an additive or affine EC-LCG recurrence. Recovering the generator state from a small number of consecutive truncated outputs lets the attacker predict all future outputs and exploit any protocol that uses those outputs as branch choices.

### Recurrence forms

- Additive: `W_{n+1} = W_n + G` (fixed step — most common in CTFs)
- Affine: `W_{n+1} = a·W_n + B` (scalar multiply then add)
- Output: `id_n = (x(W_n) >> k, y(W_n) >> k)` — top bits of both coordinates

### Small-window recovery (bounded search, k ≤ ~16 bits hidden)

For small `k`, try all `2^k` low-bit completions for each leaked coordinate, lift to the curve, then validate the recurrence against the next observation.
...
G = E(<Gx>, <Gy>)
k_bits = 32          # bits hidden per coordinate

def candidates_from_x_high(x_high):
    """Yield all curve points whose x-coordinate has x_high as top bits."""
    for low in range(2**k_bits):
        x = (x_high << k_bits) | low
        try:
            pt = E.lift_x(GF(p)(x))           # raises if not on curve
            yield pt
            yield -pt                           # both y solutions
        except Exception:
            pass

# Recover W_n given six consecutive high-bit pairs (additive recurrence)
def crack_ec_lcg_additive(high_coords):
    # high_coords = [xh0, yh0, xh1, yh1, ...]  — consecutive id pairs
    # Try all completions for the first point
    x_high_0, y_high_0 = high_coords[0], high_coords[1]
    for W0 in candidates_from_x_high(x_high_0):
        if (int(W0[1]) >> k_bits) != y_high_0:
            continue
        # Validate forward against next observed pairs
        W = W0
        valid = True
        for i in range(1, len(high_coords) // 2):
            W = W + G
            if (int(W[0]) >> k_bits) != high_coords[2*i]:
                valid = False
                break
            if (int(W[1]) >> k_bits) != high_coords[2*i+1]:
                valid = False
                break
        if valid:
            return W0
...
### Polynomial/SageMath recovery (larger hidden windows)

When the hidden window is larger, model each leaked pair as `x_hat * 2^k + dx`, apply the curve equation and recurrence to couple unknowns, then solve with `ideal.groebner_basis()` or CVP/LLL.

```python
# Sage polynomial system approach (additive recurrence, top bits known)
K = GF(p)
R = PolynomialRing(K, ['a1','b1','a2','b2','a3','b3'])
a1,b1,a2,b2,a3,b3 = R.gens()

u1,v1 = x_high_0 << k, y_high_0 << k   # observed high bits (shifted back)
# Curve equation: y^2 = x^3 + ax + b  where x = u + a_i, y = v + b_i
eq1 = (v1+b1)**2 - (u1+a1)**3 - curve_a*(u1+a1) - curve_b
# Point addition W_1 = W_0 + G relates (u1+a1, v1+b1) to (u2+a2, v2+b2)
# Build 6 equations in 6 unknowns, solve via Groebner basis
I = R.ideal([eq1, ...])
solutions = I.variety()
```

### Predicting future outputs (ZKP / branch use-case)

Once `W_current` is recovered, advance the recurrence locally to predict all future branch bits before opening an exploit session against an interactive protocol.

```python
# Advance additive EC-LCG and extract branch bits
def predict_branches(W_start, G, n_steps, bit_fn=lambda pt: int(pt[0]) & 1):
    """
    bit_fn: how the service extracts a branch bit from a point.
    Typical: low bit of x-coordinate, or (x >> k) & 1.
    """
    W = W_start
    schedule = []
    for _ in range(n_steps):
        W = W + G
        schedule.append(bit_fn(W))
    return schedule
```

### Key pitfalls

- **Off-by-one state alignment:** the service may emit IDs starting at `W_1`, not `W_0`. Try both offsets before blaming the recurrence math.
- **Separate RNG instances:** if IDs and branch bits come from different instances, state recovery from IDs alone will not predict branches.
- **Re-seeding per connection:** recover and exploit within a single session; a new connection re-seeds the generator.
- **y-coordinate ambiguity:** `lift_x` returns both `y` and `-y`; test both against the next observed pair.
