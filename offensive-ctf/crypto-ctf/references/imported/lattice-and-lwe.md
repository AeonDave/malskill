...
- in lab tasks, `BKZ(block_size=20..35)` is often already enough
...
**Key insight:** HNP attacks usually do not require a perfect lattice model. In lab tasks, once the true secret produces a vector much shorter than random noise, `LLL` often exposes it directly or gets you close enough to brute-force the last few bits.
...
In many lab tasks, the intended shortcut is not a full Ring-LWE attack, but one of these:
...
**Key insight:** most Ring-LWE / Module-LWE challenges are weakened by implementation mistakes, tiny errors, or over-structured secrets. Flatten to plain LWE first and check whether standard lattice tools solve it before pursuing ring-specific attacks.
