...
- [references/ground-systems-and-rf.md](references/ground-systems-and-rf.md) — SDR reception and `gr-satellites` decode chains, AX.25/KISS, CubeSat CSP header (v1/v2, ports, libcsp, GOMspace modems), TLE/SGP4 orbital mechanics, ground-segment software as attack surface, weather-image downlinks (APT / SSTV), **GNSS spoofing with `gps-sdr-sim` + HackRF** (jam-then-spoof, sample-rate matching, multi-band caveats, OSNMA/meaconing), **ZMQ "SDR modem" endpoints** (ZMTP socket-type identification, proving the peer actually consumes, stream-vs-PMT payloads), **pass-window/sample-grid rules** for graded az/el answers, and **Hack-A-Sat winning patterns** (pointing-→-sat-ID, ground-→-space commanding, ADCS control-constant abuse, RF RE flow). Load for RF/IQ inputs, CubeSat links, orbit math, a ZMQ/SDR modem interface, ground-system targets, or GNSS-receiver challenges.
