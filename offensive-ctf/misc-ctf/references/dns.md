...
- [DNS Maze Traversal]
- [TCP Fast Open SYN-Payload Command Injection]

-

## EDNS Client Subnet (ECS) Spoofing
**Pattern:** DNS server returns different records based on client IP. Spoof source using ECS option.
...
## DNSSEC NSEC Walking
**Pattern:** NSEC records in DNSSEC zones reveal all domain names by chaining to the next name.
...
**Pattern:** When AXFR is blocked, IXFR from old serial reveals zone update history including deleted records.
