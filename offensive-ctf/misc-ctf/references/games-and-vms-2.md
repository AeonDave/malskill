# Preserved source: games-and-vms-2.md
...
**Pattern:** Server-side game where selecting tiles increases score. Incorrect choice resets the game. Score tracked via session cookies.
...
**Pattern:** Flask game stores correct answers in signed session cookies. Use `flask-unsign -d` to decode the cookie and reveal server-side game state without playing.
...
**Pattern:** Browser-based maze game with invisible walls. Checkpoints verified server-side via WebSocket. Cryptic hint encodes target coordinates.
...
**Pattern:** Side-scrolling game requiring traversal of a map. Server validates that enough time has elapsed (map_length / speed) but doesn't verify actual movement.
...
**Pattern:** Server generates random n-bit binary code each round. Input must contain the code as a substring. Pass 20+ rounds with a single fixed input under a character limit.
...
**Pattern:** Large Brainfuck program (10K+ instructions) validates a flag character-by-character. Full reverse engineering is impractical.
...
**Pattern:** Browser game compiled to WebAssembly with win conditions requiring luck (e.g., 15 consecutive coin flips). WASM linear memory is flat and unprotected.
...
## References
- "Signal from the Deck": Cookie checkpoint game brute-forcing
- "Hungry, Not Stupid": Flask cookie game state leakage
- "Maze of the Unseen": WebSocket teleportation + cryptic hints
- "Level Devil": Server time-only validation bypass
- "Brown's Revenge": De Bruijn sequence substring coverage
- "Ghost Ship": Brainfuck instrumentation brute-force
- "Dubious Doubloon": WASM linear memory state patching
