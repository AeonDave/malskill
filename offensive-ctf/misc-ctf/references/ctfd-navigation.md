# Preserved source: ctfd-navigation.md
...
**Important:** When CTFd is detected, **ask the user for their API token**. Tokens are not provided by default — the user must generate one
...
| Connection info not in API | Some lab tasks use dynamic instances | Check for challenge-specific instance API or Docker endpoints |
