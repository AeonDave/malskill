# Preserved source: encodings-advanced.md
...
- [TOPKEK Binary Encoding]
- [MaxiCode 2D Barcode Decoding]
- [DTMF Audio with Multi-Tap Phone Keypad Decoding]
- [Music Note Interval Steganography]
- [Ruby Array#unpack Buffer Under-Read CVE-2018-8778]
- [Binary Grid Text to QR Image + XOR Key]
...
**Pattern:** Web interface with a circular wheel (5 concentric circles = 5 bits, 32 positions). Must fill in a valid Gray code sequence where consecutive values differ by exactly one bit.
...
**Pattern:** Data hidden inside custom RTF control sequences. Extract numbered blocks, sort by index, concatenate, and base64-decode.
...
**Key insight:** Look for "4042" or "UTF-9" in challenge descriptions. The April Fools' RFC series (RFC 1149, 2549, 4042) occasionally appears in lab tasks.
...
**Pattern:** Flag is encoded across 4 QR codes, each containing one quadrant of a 16x16 hexadecimal Sudoku grid. Solve the Sudoku, read the main diagonal values as hex pairs, convert to ASCII for the flag.
...
- Known flag prefix/suffix at start/end reveals the alphabet mapping
...
**Key insight:** Known plaintext at the start and end reveals the encoding alphabet — map the known characters back to their note pairs to confirm the scale-degree assignment. Musical scale degree = nibble value; pairs of notes = one byte.
