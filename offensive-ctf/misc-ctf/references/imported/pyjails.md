...
- [func_globals to Module Chain Traversal]
- [Restricted Charset Number Generation]
- [Multi-Stage Payload with Class Attribute Persistence]
- [dir() Attribute Lookup Escape Bypassing __class__ Blocklist]
- [Restricted vim Escape via K (man) to:!sh]
- [Python Name Mangling and Attribute Access]
- [Decorator-Based Escape]
...
- [Restricted Character Repunit Decomposition]
- [Python eval() Jail Escape via Tuple Injection]
- [Python f-string Config Injection via Stored eval]
...
def payload():... # code to execute (still subject to charset/AST restrictions)
```
...
**Pattern:** Server asks for a quine (program that prints its own source code), validates it by running in a subprocess, then `exec()`s it in the main process with different globals.
...
**Pattern:** Pick exactly 2 characters for your entire expression. Server evaluates `eval(long_to_bytes(eval(expr)))` — the outer eval runs the decoded Python code.
...
**Pattern:** Sandbox launches a locked-down `vim` with `:shell`/`:!` mapped out and a secure-mode profile. Command-mode escapes are blocked, but normal-mode `K` (look up keyword under cursor via `keywordprg`, default `man`) still works. `man` internally paginates via `less`, and `less` itself has a documented shell-escape: typing `!sh` from the pager spawns a shell with the user's real privileges.
