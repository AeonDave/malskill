...
Use this reference when the concise `SKILL.md` routing is not enough and the agent needs a step-by-step solve loop for reproducible challenge writeup generation.
...
3. **Technique handoff** — load the first matching methodology skill: `forensic-technique`, `evidence-before-claims`, `verification-before-completion`.
4. **Tool selection** — choose from: `coding/test-driven-development`, `knowledge/evidence-before-claims`, `knowledge/verification-before-completion`.
