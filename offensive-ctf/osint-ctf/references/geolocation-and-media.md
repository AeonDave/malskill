...
**Pattern:** Encoded coordinates like "4V FH 246 677".
...
**Pattern:** Flag format requires a Google Plus Code (e.g., `H9G2+47X`) instead of coordinates or W3W. Plus Codes are Google's open-source alternative to street addresses.
...
**Pattern:** Battery specifications -> manufacturer identification. Cross-reference specs (voltage, capacity, form factor) with manufacturer databases.
...
**Pattern:** Combine newspaper archive date search with EXIF GPS coordinates for location-specific identification.

**Tools:** Library of Congress newspaper archive, Google Maps for GPS coordinate lookup.

## Google Street View Panorama Matching

**Pattern:** Challenge image is a cropped section of a Google Street View panorama. Must identify the exact panorama ID and coordinates.
...
**Pattern:** Street view image of a coastal location. Identify exact coordinates from road infrastructure.
...
**Pattern:** Coastal parking lot image. Identify location from architectural style, vehicle types, signage, and local brands.
...
**Pattern:** Challenge image contains multiple elements but only one is useful for identification. Crop to just the relevant portion before searching.
...
3. Search the cropped region using Google Lens
...
**Pattern:** Text visible in the image is reflected/mirrored (e.g., sign reflected in water or glass). Must read the text in reverse to identify the location.
...
# The W3W API requires an API key:
...
**Pattern:** Photo of large 3D letters spelling a city/location name, often reflected in a pool of water. Common in Latin American cities as tourist landmarks.
...
**Pattern:** Verify a candidate location by matching a challenge image against user-submitted Google Maps photos for that place.
...
**Pattern:** Find a specific business (newsagent) near a metro entrance in a known city. Overpass Turbo queries OpenStreetMap data to locate POIs by type within a radius of other POIs.
...
**Pattern:** 14 images of music-themed landmarks worldwide. For each location:
...
**When to recognize:** Artifact set provides multiple images with a musical or thematic thread. Each image requires individual geolocation. The flag isn't at any single location — it's encoded across all of them.
