...
**Pattern:** Find target's posts on BlueSky social media.
...
- [Osint Industries](https://osint.industries) - Cross-platform people search
...
Cross-reference extracted codes with postal code databases, phone number registries, or geographic gazetteers to narrow the subject's location.
...
**Pattern:** Reddit username -> Spotify social link -> Base58-encoded string -> Spotify playlist descriptions (base64) -> first-letter acrostic from song titles.
...
**Pattern:** Flag hidden in a Bluesky post using Unicode homoglyph steganography — visually identical characters from different Unicode blocks encode binary data.
...
curl -s "https://public.api.bsky.app/xrpc/app.bsky.feed.searchPosts?q=+flash+ctf&sort=latest" | jq '.posts[].record.text'

# Search for specific accounts
curl -s "https://public.api.bsky.app/xrpc/app.bsky.actor.searchActors?q=" | jq '.actors[].handle'

# Get profile
curl -s "https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=.bsky.social" | jq

# Get author feed (all posts)
curl -s "https://public.api.bsky.app/xrpc/app.bsky.feed.getAuthorFeed?actor=.bsky.social&limit=50" | jq '.feed[].post.record.text'
...
**Pattern:** Target's physical location identified through fitness tracking data. Username discovered on Twitter → alias found in GitHub code → alias searched on Strava → running route endpoint reveals location.
...
**Detection:** the prompt mentions exercise, running, cycling, fitness, GPS, or health tracking. Target persona has an active/athletic profile.

-

## Discord API Enumeration

**Pattern:** Flags hidden in Discord server metadata not visible in normal UI.
